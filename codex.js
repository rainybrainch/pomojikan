...
function buildKanjiCodex(){
  ...
  // 16段階の文字種＋学年別段階解放（v3 / 2026-05-16）
  // 順番が大切：先に登録された rarity が優先（seen.has で重複判定）
  addStr(HIRAGANA_STR, '★1',  ['ひらがな','音']);
  addStr(KATAKANA_STR, '★2',  ['カタカナ','音']);
  addStr(NUMERAL_STR,  '★3',  ['数字','数']);
  addStr(ALPHABET_STR, '★4',  ['英語','異邦']);
  addStr(ROMAN_STR,    '★5',  ['ローマ数字','古代']);
  // 文科省学年別漢字配当表（1026字）
  addStr(KANJI_G1_STR, '★6',  ['漢字','小一','基礎']);
  addStr(KANJI_G2_STR, '★7',  ['漢字','小二','日常']);
  addStr(KANJI_G3_STR, '★8',  ['漢字','小三']);
  addStr(KANJI_G4_STR, '★9',  ['漢字','小四']);
  addStr(KANJI_G5_STR, '★10', ['漢字','小五']);
  addStr(KANJI_G6_STR, '★11', ['漢字','小六']);
  // 中学（既存「五級」相当）・高校（既存「三級」相当）・人名・古典・神字
  addStr(KANJI_5KYU_STR,   '★12', ['漢字','中学','常用']);
  addStr(KANJI_3KYU_STR,   '★13', ['漢字','高校','深']);
  addStr(KANJI_1KYU_STR,   '★14', ['漢字','人名','美']);
  addStr(KANJI_SHODAN_STR, '★15', ['漢字','古典','古字']);
  addStr(KANJI_JUDAN_STR,  '★16', ['漢字','神字','七徳七大罪']);
  return all;
}
